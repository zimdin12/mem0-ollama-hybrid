...
## Brain Agent (v2) Issues

**Symptom**: `memory_agent` returns generic error

1. Check brain status: `curl http://localhost:8765/api/v1/brain/status`
2. Check audit log: `curl http://localhost:8765/api/v1/brain/audit?limit=5`
3. Model may be overloaded — check `docker exec ollama ollama ps`

**Symptom**: Agent loops without finishing

Default max steps is 12 (`BRAIN_MAX_STEPS`). If the agent hits this limit, the query may be too complex. Try breaking it into simpler requests.
