...
**Two versions exist as separate branches:**

| Branch | Interface | Best for |
|--------|-----------|----------|
| `main` (v1) | 7 specific tools (`search_memory`, `add_memories`, etc.) | Fast programmatic access, auto-recall hooks |
| `memory_agent` (v2) | 1 natural language tool (`memory_agent`) + all v1 tools | Chat clients, complex queries, natural conversation |
...
### v2 (memory_agent branch only)
```
POST /api/v1/brain             — natural language memory operations
GET  /api/v1/brain/audit       — audit log
GET  /api/v1/brain/status      — brain config
```
...
### v2 Tool (memory_agent branch only)
| Tool | Purpose |
|------|---------|
| `memory_agent` | Natural language — search, store, delete, update, explore |
...
|----------|---------|---------|
| `LLM_MODEL` | `qwen3:4b-instruct-2507-q4_K_M` | Extraction + brain model |
